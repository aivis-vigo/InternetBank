...
@section('content')
    <div class="mx-auto w-1/2 my-4">
        <div class="flex text-center justify-between m-2">
            ...
            <div class="flex-1">Circulating Supply</div>
        </div>
                ...
                <a href="/coin/{{ $coin->id }}">
                    <div class="flex justify-between text-center p-2 my-2 w-full hover:bg-gray-300 hover:rounded-lg hover:transition duration-200">
                        ...
                        <div class="flex flex-1 gap-x-2">
                            <div class="flex">
                                <img src="https://s2.coinmarketcap.com/static/img/coins/16x16/{{ $coin->id }}.png" class="object-contain">
                            </div>
                            <div>{{$coin->name}}</div>
                            <div class="text-gray-500">{{ $coin->symbol }}</div>
                        </div>
                        <div class="flex-1">{{ number_format($coin->quote->EUR->price, 2) }}</div>
                        <div class="flex-1">{{ number_format($coin->total_supply, 2) }} {{ $coin->symbol }}</div>
                    </div>
                </a>
